...
static	vec3_t	forward; //Boot: Why is this ambigious?
...
//	self->s.weapon = WP_EMPLACED_GUN; // we need to be able to switch back to the old weapon
	...
	ugly[ROLL] = crandom() * 7;
	gi.G2API_SetBoneAnglesIndex( &self->ghoul2[self->playerModel], self->lowerLumbarBone, ugly, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, -1, -1);
...
void SP_emplaced_gun( gentity_t *ent )
{
	...
	gi.G2API_SetBoneAngles( &ent->ghoul2[0], "swivel_bone", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, -1, -1);
