...
static qboolean PM_CheckJump( void ) 
{
	...
	*/
	...
	{//in low grav, you push in the dir you're facing as long as there is something behind you to shove off of
		...
		pm->trace( &trace, pm->ps->origin, pm->mins, pm->maxs, back, pm->ps->clientNum, pm->tracemask, (EG2_Collision)0, 0);
...
void PM_SetWaterHeight( void )
{
	...
	gi.trace( &trace, top, pm->mins, pm->maxs, bottom, pm->ps->clientNum, (CONTENTS_WATER|CONTENTS_SLIME), (EG2_Collision)0, 0);
...
void PM_ResetAnkleAngles( void )
{
	...
	{
		gi.G2API_SetBoneAnglesIndex( &pm->gent->ghoul2[0], pm->gent->footLBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, NEGATIVE_X, NULL, -1, -1 ); 
	}
	if ( pm->gent->footRBone != -1 )
	{
		gi.G2API_SetBoneAnglesIndex( &pm->gent->ghoul2[0], pm->gent->footRBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, NEGATIVE_X, NULL, -1, -1 ); 
	}
}
	...
	*/
	...
	if ( pm->gent->client->NPC_class == CLASS_ATST )
	{
		...
		{//rotate the ATST's left foot pad to match the slope
			...
			//Hmm... lerp this?
			gi.G2API_SetBoneAnglesIndex( &pm->gent->ghoul2[0], pm->gent->footLBone, footAngles, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, NEGATIVE_X, NULL, -1, -1 ); 
		}
		...
		{//rotate the ATST's right foot pad to match the slope
			...
			//Hmm... lerp this?
			gi.G2API_SetBoneAnglesIndex( &pm->gent->ghoul2[0], pm->gent->footRBone, footAngles, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, NEGATIVE_X, NULL, -1, -1 ); 
		}
	}
...
static void PM_WaterEvents( void ) {		// FIXME?
	...
	if ( impact_splash )
	{
		...
		gi.trace( &tr, start, vec3_origin, vec3_origin, end, pm->gent->s.number, CONTENTS_WATER, (EG2_Collision)0, 0);
